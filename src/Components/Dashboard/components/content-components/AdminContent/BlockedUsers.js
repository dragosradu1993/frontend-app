...
export default function BlockedUsers(props) {
    ...
    const handleClick = async (event, index) => {
      ...
      if(button.type === 'unblock-user')  {
            const sendData = await utils.user.unblockUser(selectedItem[0].email)
            if(!sendData.status) {
              setGridAlert({isError: true, severity: 'error', textError: sendData.message})    
            } else {
              setGridAlert({isError: true, severity: 'success', textError: sendData.message}) 
              setGridRows([])
            } 
                  ...
                  {data.dataGrid.settings.show ? (
                    <DataGrid
                      ...
                      onSelectionModelChange={(ids) => {
                          ...
                          const selectedRowData = gridRows.filter((row) => selectedIDs.has(row.id))
