...
export default function NoProjectStudents(props) {
    ...
    const [secretaryType, setSecretaryType] = React.useState()
    let initial = true
      ...
      function NoDataOverlay() {
        return (
          <StyledGridOverlay>
            ...
            <Box sx={{ mt: 1 }}>Nu există date</Box>
          </StyledGridOverlay>
        );
      }
    ...
    const handleClick = async (event, index) => {
      ...
      if(button.action === 'refresh') {
        const proj = await utils.user.projects.noProjectStudents({
    ...
    React.useEffect(() => {
      const getData = async (props) => {
        if(props.data.hasOwnProperty('page')) {
          ...
          if(initial) {
            setSecretaryType(props.secretaryType)
